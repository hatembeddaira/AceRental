...
{
    public class B2B_LogisticStatusTests
    {
        ...
        public void B2B_Should_Always_Block_Invalid_Transitions_From_List(LogisticStatus illegalTarget)
        {
        ...
        public void B2B_Should_Can_Transition_From_Draft_To_Quote(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Draft, financial);
            // Act:
            bool result = res.LogisticStatus.CanTransitionTo(LogisticStatus.Quote, res);
            // Assert
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Draft_To_Deleted(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Draft, financial);
            // Act:
        ...
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Draft(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
                // Arrange
                var res = CreateReservation(LogisticStatus.Draft, fin);
                ...
                // Assert
                Assert.False(canTransition, 
                    $"Erreur : La transition Draft -> {illegalTarget} devrait être interdite pour le statut financier {fin}");
            }
        }
        #endregion
        #region Transition From Quote 
        [Theory]
        [InlineData(FinancialStatus.Unpaid, true)]
        [InlineData(FinancialStatus.PartiallyPaid, false)]
        [InlineData(FinancialStatus.Paid, false)]
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Quote_To_Confirmed(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Quote, financial);
            // Act:
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Quote_To_Cancelled(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Quote, financial);
            // Act:
        ...
        [InlineData(LogisticStatus.Damaged)]
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Quote(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
                // Arrange
                var res = CreateReservation(LogisticStatus.Quote, fin);
                ...
                // Assert
                Assert.False(canTransition, 
                    $"Erreur : La transition Quote -> {illegalTarget} devrait être interdite pour le statut financier {fin}");
            }
        }
        #endregion
        ...
        public void B2B_Should_Can_Transition_From_Cancelled_To_Draft(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Cancelled, financial);
            // Act:
            bool result = res.LogisticStatus.CanTransitionTo(LogisticStatus.Draft, res);
            // Assert
            Assert.Equal(excepted, result);
        }
        ...
        [InlineData(LogisticStatus.Damaged)]
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Cancelled(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Confirmed_To_Cancelled(FinancialStatus financial, bool excepted)
        {
            // Arrange
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Confirmed_To_PickedUp(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Confirmed, financial);
            // Act:
        ...
        [InlineData(LogisticStatus.Damaged)]
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Confirmed(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_PickedUp_To_Returned(FinancialStatus financial, bool excepted)
        {
            // Arrange
        ...
        [InlineData(LogisticStatus.Damaged)]
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_PickedUp(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Returned_To_Checked(FinancialStatus financial, bool excepted)
        {
            // Arrange
        ...
        [InlineData(LogisticStatus.Damaged)]
        [InlineData(LogisticStatus.Finished)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Returned(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Checked_To_Damaged(FinancialStatus financial, bool excepted)
        {
            // Arrange
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Checked_To_Finished(FinancialStatus financial, bool excepted)
        {
            // Arrange
            var res = CreateReservation(LogisticStatus.Checked, financial);
            // Act:
        ...
        [InlineData(LogisticStatus.Returned)]
        [InlineData(LogisticStatus.Checked)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Checked(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
        ...
        [InlineData(FinancialStatus.RentalInvoiced, false)]
        [InlineData(FinancialStatus.Refunded, false)]
        public void B2B_Should_Can_Transition_From_Damaged_To_Finished(FinancialStatus financial, bool excepted)
        {
            // Arrange
        ...
        [InlineData(LogisticStatus.Checked)]
        [InlineData(LogisticStatus.Damaged)]
        public void B2B_Should_Always_Block_Invalid_Transitions_From_Damaged(LogisticStatus illegalTarget)
        {
            // On boucle sur TOUS les statuts financiers possibles
            foreach (FinancialStatus fin in Enum.GetValues<FinancialStatus>())
            {
